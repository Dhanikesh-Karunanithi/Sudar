...
import { ADMIN_AI_LITERACY_LESSONS } from '@/content/adminAiLiteracyCourse'

export default function AiAtSudarCoursePage() {
  const [activeId, setActiveId] = useState(ADMIN_AI_LITERACY_LESSONS[0]?.id ?? '')
  const lesson = ADMIN_AI_LITERACY_LESSONS.find((l) => l.id === activeId) ?? ADMIN_AI_LITERACY_LESSONS[0]
  const idx = ADMIN_AI_LITERACY_LESSONS.findIndex((l) => l.id === activeId)
  const progressPct =
    ADMIN_AI_LITERACY_LESSONS.length > 0
      ? Math.round(((idx + 1) / ADMIN_AI_LITERACY_LESSONS.length) * 100)
      : 0
    ...
    <div className="p-6 lg:p-8 max-w-6xl mx-auto">
      ...
      <div className="mb-6 rounded-lg border border-slate-800 bg-slate-900/40 p-3">
        <div className="flex justify-between text-xs text-slate-500 mb-1">
          <span>Progress</span>
          <span>
            Lesson {idx + 1} of {ADMIN_AI_LITERACY_LESSONS.length}
          </span>
        </div>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        <nav className="lg:w-64 shrink-0 space-y-1">
          ...
          {ADMIN_AI_LITERACY_LESSONS.map((l, i) => (
            <button
              key={l.id}
              type="button"
        ...
        <article className="flex-1 min-w-0 rounded-xl border border-slate-800 bg-slate-900/50 p-6 lg:p-8">
          ...
          {lesson.sections.map((sec, si) => (
            <div key={si} className="mb-6 last:mb-0">
              {sec.heading ? (
                <h3 className="text-base font-medium text-slate-200 mt-0 mb-3">{sec.heading}</h3>
              ) : null}
      ...
      <div className="mt-8 rounded-lg border border-slate-800 bg-slate-900/30 p-4 flex items-start gap-3">
        ...
        <p className="text-sm text-slate-500">
          Printable reference for operators: see <code className="text-slate-400">docs/admin/AI_LITERACY_AND_LOCAL_MODELS.md</code> in the Sudar repository.
        </p>
      </div>
    </div>
  )
