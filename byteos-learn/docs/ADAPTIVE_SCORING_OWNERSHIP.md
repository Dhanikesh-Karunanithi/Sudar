...
**Learner-facing adaptive behavior that ships today is implemented in Sudar Learn (`byteos-learn`), not in the Python `byteos-intelligence` service.**
...
- The **`byteos-intelligence`** FastAPI routes under `learner` profile update and `next-action` are **stubs** (placeholder responses). They must not be treated as the live adaptive engine until implemented and wired to the same Supabase contracts.
...
- `byteos-intelligence/src/api/routes/learner.py` (`update_learner_profile`, `compute_next_action`)
