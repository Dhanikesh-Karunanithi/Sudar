...
export default async function CourseDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: enrollment }, { data: completedEvents }] = await Promise.all([
    user
      ? admin
          .from('enrollments')
      ...
      <div className="relative overflow-hidden rounded-card-xl border border-primary/20 bg-primary/5">
        {course.banner_url ? (
          <div className="relative h-40 w-full sm:h-48">
            {/* Plain img avoids Worker image-optimizer CPU/memory on Cloudflare OpenNext */}
        ...
        <div className={cn('p-8', course.banner_url && '-mt-12 relative')}>
          <div className="flex items-start gap-5">
            <div className="w-14 h-14 rounded-card bg-card shadow-sm border border-border flex items-center justify-center shrink-0 overflow-hidden">
