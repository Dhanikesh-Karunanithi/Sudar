...
type VideoTelemetry = { courseId: string; moduleId: string }
...
export function CourseVideoCard({
  ...
  const sceneDurations = scenes.map((s) => Math.max(0.5, s.duration ?? 5))
  const totalDurationSeconds = sceneDurations.reduce((sum, d) => sum + d, 0)
  const sceneStartTimes = sceneDurations.reduce<number[]>((acc, dur, idx) => {
    if (idx === 0) return [0]
    acc.push((acc[idx - 1] ?? 0) + sceneDurations[idx - 1])
    return acc
  }, [])
  ...
  const prevPlayingTelemetryRef = useRef(false)
  ...
  const advanceScene = useCallback(() => {
    ...
    if (next < scenes.length) {
      ...
      setAnimKey((k) => k + 1)
      setElapsedSeconds(sceneStartTimes[next] ?? 0)
    } else {
      setIsPlaying(false)
      setElapsedSeconds(totalDurationSeconds)
    }
  }, [scenes.length, sceneStartTimes, totalDurationSeconds])
  ...
  const startScenePlayback = useCallback((idx: number, play: boolean) => {
    ...
    if (s?.audioDataURL) {
      ...
      audio.ontimeupdate = () => {
        ...
        setElapsedSeconds((sceneStartTimes[idx] ?? 0) + (audio.currentTime || 0))
      }
      audio.onended = () => {
        setProgress(100)
  ...
  }, [scenes, isMuted, volume, clearTimer, stopAudio, advanceScene])
  ...
  function startFallbackTimer(duration: number) {
    ...
    progressRef.current = setInterval(() => {
      ...
      setElapsedSeconds((sceneStartTimes[currentIndexRef.current] ?? 0) + elapsed / 1000)
    }, 100)
  ...
  useEffect(() => {
    const handleFullscreenChange = () => {
      ...
      setIsFullscreen(currentFullscreenEl === containerRef.current)
    }
  ...
  useEffect(() => {
    if (!telemetry) return
    if (phase === 'intro') {
      prevPlayingTelemetryRef.current = false
      return
    }
    if (isPlaying === prevPlayingTelemetryRef.current) return
    prevPlayingTelemetryRef.current = isPlaying
    const base = {
      course_id: telemetry.courseId,
      module_id: telemetry.moduleId,
      modality: 'video' as const,
      payload: { scene_index: currentIndex, scene_count: scenes.length },
    }
    if (isPlaying) {
      postLearningEvent({ event_type: 'video_play', ...base })
    } else {
      postLearningEvent({ event_type: 'video_pause', ...base })
    }
  }, [isPlaying, phase, telemetry, currentIndex, scenes.length])
  ...
  const handlePrev = () => {
    if (telemetry && currentIndex > 0) {
      ...
      postLearningEvent({
        ...
        module_id: telemetry.moduleId,
        modality: 'video',
        payload: { scene_from: currentIndex, scene_to: prev },
      })
    }
  ...
  const handleFullscreenToggle = async () => {
    ...
    try {
      if (currentFullscreenEl === container) {
      ...
      // Ignore browser-level fullscreen failures (permissions, unsupported environments, etc.)
    }
  }

  const animType = pickAnimation(scene, currentIndex)
  const totalProgressPct = totalDurationSeconds > 0
    ? Math.min(100, Math.max(0, (elapsedSeconds / totalDurationSeconds) * 100))
    : 0

  const seekToTime = (seconds: number) => {
    const target = Math.max(0, Math.min(totalDurationSeconds, seconds))
    let targetIndex = scenes.length - 1
    for (let i = sceneStartTimes.length - 1; i >= 0; i -= 1) {
      if (target >= sceneStartTimes[i]) {
        targetIndex = i
        break
      }
    }
    const start = sceneStartTimes[targetIndex] ?? 0
    const within = Math.max(0, target - start)
    const duration = sceneDurations[targetIndex] ?? 1
    clearTimer(); stopAudio()
    setCurrentIndex(targetIndex)
    setAnimKey((k) => k + 1)
    setProgress(Math.min(100, (within / duration) * 100))
  ...
  return (
    <>
      ...
      <div
        ref={containerRef}
        className="video-shell relative w-full rounded-2xl overflow-hidden border border-border shadow-xl"
        style={{
          minHeight: isFullscreen ? '100vh' : '480px',
          height: isFullscreen ? '100vh' : 'clamp(480px, calc(100vh - 260px), 680px)',
        ...
        >
          ...
          <div
            ...
            onClick={(e) => {
              ...
              seekToTime(frac * totalDurationSeconds)
            }}
          >
            <div
          ...
          <div className="hidden sm:flex items-center gap-2 shrink-0">
            <input
              ...
              onChange={(e) => {
                ...
                if (nextVolume === 0 && !isMuted) setIsMuted(true)
              }}
            ...
            <span
              ...
              style={{ color: 'var(--foreground, #555)', minWidth: '34px' }}
            >
          ...
          >
            {isFullscreen
              ? <Minimize2 className="w-4 h-4" style={{ color: 'var(--foreground, #555)' }} />
              : <Maximize2 className="w-4 h-4" style={{ color: 'var(--foreground, #555)' }} />}
          </button>
        </div>
      </div>
    </>
