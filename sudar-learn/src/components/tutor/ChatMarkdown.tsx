...
function parseInline(text: string): React.ReactNode {
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    if (match[2] && match[3]) {
      ...
      parts.push(
        isSafe ? (
          <Link key={match.index} href={href} className="font-medium text-primary underline-offset-2 hover:underline">
            {match[2]}
          </Link>
        ) : (
      ...
      parts.push(<em key={match.index} className="italic opacity-90">{match[5]}</em>)
    } else {
      parts.push(
...
export function ChatMarkdown({ text, className }: Props) {
  ...
  const blocks = parseBlocks(cleanedText.split('\n'))
