...
import { loadSkillGapSummary } from '@/lib/learner/syncTopicSkills'
...
export async function loadPersonalizationMemoryForCourse(
  ...
  const skillGapSnippet = await loadSkillGapSummary(admin, userId)
...
${difficultyComfort ? `- Difficulty comfort: ${difficultyComfort}` : ''}${skillGapSnippet}`
