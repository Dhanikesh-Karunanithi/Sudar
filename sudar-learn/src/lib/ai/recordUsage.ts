/**
 * Fire-and-forget AI usage ledger writes (Learn).
 */
...
export function recordAiUsage(admin: UsageAdmin, input: RecordAiUsageInput): void {
  void recordAiUsageAsync(admin, input).catch(() => {
    /* metering must not break learner UX */
  })
}
...
async function recordAiUsageAsync(admin: UsageAdmin, input: RecordAiUsageInput): Promise<void> {
  ...
  if (input.userId && usage?.total_tokens) {
    const today = new Date().toISOString().slice(0, 10)
    await admin.rpc('increment_usage_token_count', {
...
/** Non-LLM usage (TTS chars, images, video jobs, embedding estimates). */
