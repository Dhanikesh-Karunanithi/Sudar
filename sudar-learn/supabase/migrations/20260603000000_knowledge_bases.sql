...
-- Mirror of supabase/migrations/20260603000000_knowledge_bases.sql for Learn deploy workflows.
...
CREATE POLICY knowledge_bases_org_member_select
  ...
  USING (EXISTS (SELECT 1 FROM public.org_members om WHERE om.org_id = knowledge_bases.org_id AND om.user_id = auth.uid()));

CREATE POLICY knowledge_bases_org_admin_insert
  ON public.knowledge_bases FOR INSERT TO authenticated
  WITH CHECK (EXISTS (SELECT 1 FROM public.org_members om WHERE om.org_id = knowledge_bases.org_id AND om.user_id = auth.uid() AND om.role IN ('ADMIN', 'MANAGER', 'CREATOR')));

CREATE POLICY knowledge_bases_org_admin_update
  ON public.knowledge_bases FOR UPDATE TO authenticated
  USING (EXISTS (SELECT 1 FROM public.org_members om WHERE om.org_id = knowledge_bases.org_id AND om.user_id = auth.uid() AND om.role IN ('ADMIN', 'MANAGER', 'CREATOR')));

CREATE POLICY knowledge_bases_org_admin_delete
  ON public.knowledge_bases FOR DELETE TO authenticated
  USING (EXISTS (SELECT 1 FROM public.org_members om WHERE om.org_id = knowledge_bases.org_id AND om.user_id = auth.uid() AND om.role IN ('ADMIN', 'MANAGER')));

CREATE POLICY kb_ingest_queue_org_member_select
  ON public.kb_ingest_queue FOR SELECT TO authenticated
  USING (
    uploaded_by = auth.uid()
    OR EXISTS (SELECT 1 FROM public.org_members om WHERE om.org_id = kb_ingest_queue.org_id AND om.user_id = auth.uid() AND om.role IN ('ADMIN', 'MANAGER', 'CREATOR'))
  );

CREATE POLICY kb_ingest_queue_org_editor_insert
  ON public.kb_ingest_queue FOR INSERT TO authenticated
  WITH CHECK (
    uploaded_by = auth.uid()
    AND EXISTS (SELECT 1 FROM public.org_members om WHERE om.org_id = kb_ingest_queue.org_id AND om.user_id = auth.uid() AND om.role IN ('ADMIN', 'MANAGER', 'CREATOR', 'LEARNER'))
  );
