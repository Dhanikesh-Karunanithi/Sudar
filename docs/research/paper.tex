...
and an AI tutor with longitudinal cross-session memory and proactive
...
portable; and (3)~a demonstrated \textbf{radical cost
...
game-style activities, and SCORM packages), with a modality dispatcher
that recommends formats per learner profile.
...
modality from affinity scores and recent behaviour, supporting
dual-coding principles \cite{mayer2009,clark2016}.
...
\begin{enumerate}
  ...
  \item \textbf{SudarRecommend.} A next-best-action widget for the LMS
    dashboard. The reference Learn app exposes \texttt{/api/alp/next-action}
    (authenticated) which forwards to Intelligence; internal calls use
    \texttt{/api/intelligence/next-action}. No schema changes are required
