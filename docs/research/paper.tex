...
% v4 (June 2026): arXiv tone pass, cost worksheet alignment, benchmark artefact
...
\noindent\textbf{Keywords:} adaptive learning; intelligent tutoring systems;
learner modelling; digital learner twin; open-source LMS; multimodal learning;
large language models; educational technology; plugin architecture; learning
analytics.
...
This reduction in inference cost makes AI-native learning economically
viable at population scale for the first time, yet no major LMS vendor has
...
\begin{tabular}{@{}p{1.35cm}p{1.85cm}p{1.35cm}p{1.5cm}@{}}
\toprule
...
Cost (1k/mo)   & $\sim$\$21 & N/A & \$3.4k--\$44k \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\paragraph{HTTP contract (reference).}
Connectors authenticate with \texttt{x-alp-api-key} (or Bearer) and
POST batched events to Learn, e.g.\
\texttt{\{ "user\_id": "<uuid>", "events": [\{ "event\_type":
"module\_complete", "course\_id": "...", "modality": "text",
"duration\_secs": 120 \}] \}}.
Twin updates run asynchronously after ingestion; tutor and
next-best-action calls use the same org-scoped \texttt{user\_id}.
\textbf{LTI~1.3} launch and JWKS endpoints exist on Learn for Moodle/Canvas
packaging (\texttt{/api/alp/lti/*}); vendor installable plugins remain
roadmap (see \texttt{docs/ALP\_API.md}).
...
% ---------------------------------------------------------------
\section{Economic Feasibility and Cost Analysis}
\label{sec:economics}
% ---------------------------------------------------------------

A primary barrier to AI-native adaptive learning is perceived
infrastructure cost. This section reports \emph{marginal} AI provider
cost for a defined workload (Appendix~\ref{sec:cost-appendix}, worksheet
in \texttt{COST\_WORKSHEET.md}); it is not a full total cost of ownership
(TCO) for content, support, or enterprise LMS seats.

\paragraph{Defined workload and headline cost.}
We model high engagement per learner per month: ten course-generation
runs, twenty tutor chat sessions, adaptive personalisation, mindmap and
flashcard generation, and TTS narration (Together AI 8B + Edge-TTS,
Q2 2026 list prices). The worksheet yields approximately
\textbf{\$0.021 per learner per month} (\$0.25 annualised per learner for
AI APIs only). At 1{,}000 active learners the marginal AI stack is
\textbf{\$21/month}; at 10{,}000 learners, \textbf{\$210/month}.

\paragraph{Marginal AI stack comparison.}
Table~\ref{tab:marginal-ai} contrasts \emph{like-with-like} AI inference
and TTS stacks for the same 1{,}000-learner scenario (not platform seat
licences).

\begin{table}[H]
\centering
\caption{Marginal AI infrastructure (1{,}000 learners, defined workload).}
\label{tab:marginal-ai}
\scriptsize
\resizebox{\columnwidth}{!}{%
\begin{tabular}{@{}lrr@{}}
\toprule
...
At 10{,}000 learners, the delta versus a closed-model stack in
Table~\ref{tab:marginal-ai} exceeds \$400{,}000 per year---savings from
open-weight inference rather than proprietary API pricing alone.

\paragraph{Impact of inference optimisation.}
...
Maintaining longitudinal tutor memory per learner at the marginal cost
in our worksheet (\$0.021/user/month) is therefore computationally and
economically feasible for large-scale deployments, particularly in
resource-constrained settings where proprietary enterprise licences are
often exclusionary.

\paragraph{Accessibility context.}
...
\emph{marginal} AI stack cost (\$0.021/user/month in our worksheet) is
...
Platforms priced at \$12--\$20 per user per month for bundled AI features
remain difficult to deploy in this context. Sudar's \emph{marginal} AI
stack for the workload above is approximately \$0.021 per learner per
month on open-weight inference; the reference implementation is
open-source, self-hostable, and runnable without proprietary model lock-in.

\paragraph{System performance and latency.}
\label{sec:perf}
To validate that longitudinal modelling does not block the learner UI,
we measured core paths with \texttt{scripts/benchmark-sudar.mjs}
(results in \texttt{docs/research/benchmark-results.json}).
Table~\ref{tab:perf} summarises a representative run (method: 20 samples
per endpoint, p50/p95; Intelligence \texttt{/api/health} and optional
tutor query when service credentials are configured; local heuristic
ranking microbench for catalog size $N{=}1000$).

\begin{table}[H]
\centering
\caption{Baseline latency (artefact: \texttt{benchmark-results.json}). Health/tutor
rows require a live Intelligence URL and service secret.}
\label{tab:perf}
\scriptsize
...
Path & p50 (ms) & p95 (ms) \\
\midrule
Intel.\ \texttt{/api/health} & --- & --- \\
Intel.\ \texttt{/api/tutor/query} & --- & --- \\
NBA heuristic ($N{=}1000$) & 0.2 & 5 \\
\bottomrule
\end{tabular}%
}
\end{table}
\noindent\scriptsize{Reproduce: \texttt{node scripts/benchmark-sudar.mjs}.}
...
practice and validated against current provider pricing; and
(e)~baseline latency for core paths is reported in
Section~\ref{sec:perf} with a reproducible harness.
...
learning to approximately \$0.021 per learner per month for the defined
marginal AI workload (Section~\ref{sec:economics}), supporting
deployment in resource-constrained contexts where proprietary
\$12--\$20/user/month platforms are often exclusionary.

Future work includes IRB-approved efficacy trials, hardened ALP LMS
packaging, and interpretability of modality affinity scores. The
reference implementation is open source (Apache-2.0), grounded in a
broad learning-science evidence base
\cite{vanlehn2011,aleven2016,mayer2009,clark2016,zimmerman2002,%
gagne1985,bull2016,black1998,roediger2006}, and intended as
extensible infrastructure for researchers and institutions to evaluate
and adapt.
...
\texttt{docs/sudar-memory.md}). Institutions interested in pilots or ALP
integrations may contact \texttt{connect@dhanikeshkarunanithi.com}.
...
\midrule
\multicolumn{3}{@{}l@{}}{\emph{Illustrative platform list prices (not measured):}} \\
...
\end{center}
\noindent\small{Marginal AI rows follow \texttt{COST\_WORKSHEET.md} (Q2 2026).
Platform licence rows are public list-price illustrations only---not
comparable to marginal AI cost without a full TCO study. Hosting
excluded (Supabase free tier: \$0; Pro: \$25/month). Sensitivity: 0.5$\times$/
2$\times$ usage $\Rightarrow$ \$0.011--\$0.042 per learner/month on the
8B stack.}

\section{Reproducibility and evaluation artefacts}
\noindent\small{Cost assumptions: \texttt{COST\_WORKSHEET.md}; claim scope:
\texttt{EVALUATION\_APPENDIX.md}; pilots: \texttt{PILOT\_PROTOCOL.md};
latency: \texttt{node scripts/benchmark-sudar.mjs} $\rightarrow$
\texttt{benchmark-results.json}.}
