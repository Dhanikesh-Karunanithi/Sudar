  ...
  return (
    <ProseSection title="Self-Host Sudar at $0">
      ...
      <ol className="mt-4 list-decimal space-y-2 pl-6 text-foreground">
        ...
        <li>Add environment variables from <code>sudar-studio/.env.example</code>: Supabase keys, <code>NEXTAUTH_URL</code>, <code>NEXTAUTH_SECRET</code>, <code>BYTEOS_INTELLIGENCE_URL</code>, at least one AI key, <code>NEXT_PUBLIC_LEARN_APP_URL</code> (you’ll set this after deploying Learn).</li>
      ...
      <ol className="mt-4 list-decimal space-y-2 pl-6 text-foreground">
        ...
        <li>Add env vars from <code>sudar-learn/.env.example</code>: Supabase, <code>NEXTAUTH_URL</code> (your Learn URL), <code>NEXTAUTH_SECRET</code>, <code>BYTEOS_INTELLIGENCE_URL</code>, <code>NEXT_PUBLIC_APP_URL</code> (same as NEXTAUTH_URL).</li>
        <li>Deploy.</li>
      </ol>
      <h2 className="mt-10 text-xl font-semibold text-foreground">Step 3: Deploy Sudar Intelligence</h2>
      <p className="mt-2 text-foreground">
        Vercel only runs Node.js. The Python FastAPI service in <code>sudar-intelligence/</code> must be hosted
      ...
      <p className="mt-4 text-foreground">
        Alternatives: <strong>Render</strong> (New Web Service, root <code>sudar-intelligence</code>, same build/start commands) or <strong>Fly.io</strong> (Docker or <code>fly launch</code> in <code>sudar-intelligence</code>). Free tiers may spin down after inactivity; Railway and Render paid tiers give always-on.
      </p>
