...
import { useDeviceLayout, useHeroViewport, useWireframeCinematic } from "./WireframeCinematicContext";
...
export function StudioCreateSourcesScene({ state }: { state: SceneState }) {
  ...
  const shouldType =
    (cinematic || heroViewport) && (source === "idea" || state.highlightId === "prompt");
  const typedPrompt = useTypingText(SARAH_PROMPT, shouldType, heroViewport ? 16 : 22);
      ...
      <PulseHighlight active={source === "idea" || state.highlightId === "prompt"}>
        ...
        <div className="rounded-xl border border-white/[0.08] bg-[#0d0d0d] p-3 min-h-[72px]">
          <p className="text-[11px] text-zinc-400 leading-relaxed">
            {typedPrompt}
            {shouldType && typedPrompt.length < SARAH_PROMPT.length && (
...
export function StudioLiveEditorScene({ state }: { state: SceneState }) {
  ...
  return (
    <WireframeScreen label="Sudar Studio · Editor">
      <div
        className={`grid grid-cols-1 gap-3 h-full min-h-0 overflow-hidden ${
          heroViewport ? "lg:grid-cols-[140px_1fr]" : "lg:grid-cols-[180px_1fr] gap-4"
        }`}
      >
        ...
        <div className={`space-y-2 min-h-0 ${heroViewport ? "overflow-hidden" : "space-y-3"}`}>
          {showGenerating && (
            <motion.div
