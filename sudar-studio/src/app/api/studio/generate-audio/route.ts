...
async function generateAudioDataURL(
  ...
  return (await tryOpenAI(text, openAiVoice)) ?? (await tryEdgeTTS(text, edgeVoice, authToken))
}
...
export async function POST(request: NextRequest) {
  ...
  let orgTtsVoice = VOICE_HOST_EDGE
  if (course.org_id) {
    ...
    const aiModels = (orgSettings.ai_models as Record<string, unknown>) ?? {}
    const configuredVoice = typeof aiModels.tts_voice === 'string'
      ? normalizeTtsVoiceId(aiModels.tts_voice) ?? ''
      : ''
    if (configuredVoice && TTS_VOICE_OPTIONS_BY_ID[configuredVoice]) {
      orgTtsVoice = configuredVoice
    }
  }
  ...
  if (type === 'video') {
    ...
    for (const scene of scenes) {
      ...
      const dataURL = await generateAudioDataURL(
        text,
        VOICE_VIDEO_OPENAI,
        orgTtsVoice || VOICE_VIDEO_EDGE,
        session?.access_token
      )
      if (!dataURL) failures.push(scene.sceneNumber)
      updated.push({ ...scene, audioDataURL: dataURL ?? undefined })
    }
    if (failures.length > 0) {
      return NextResponse.json(
        { error: `Audio generation failed for scenes: ${failures.join(', ')}. Check TTS provider setup and Intelligence auth.` },
        { status: 502 }
      )
    }
  ...
  for (const seg of dialogue) {
    ...
    const dataURL = await generateAudioDataURL(
      ...
      isExpert ? orgTtsVoice || VOICE_EXPERT_EDGE : orgTtsVoice || VOICE_HOST_EDGE,
      session?.access_token,
    )
    if (!dataURL) failures.push(idx + 1)
    updated.push({ ...seg, audioDataURL: dataURL ?? undefined })
    idx += 1
  }
  if (failures.length > 0) {
    return NextResponse.json(
      { error: `Audio generation failed for podcast segments: ${failures.join(', ')}. Check TTS provider setup and Intelligence auth.` },
      { status: 502 }
    )
  }
