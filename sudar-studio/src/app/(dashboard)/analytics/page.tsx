...
export default async function AnalyticsPage({
  ...
  const [
    ...
    admin.from('analytics_org_rollup').select('event_date, active_learning_secs, idle_secs, total_secs, completion_count, drop_off_count, avg_engagement_score').eq('org_id', orgId).order('event_date', { ascending: false }).limit(30),
    admin.from('analytics_risk_signals').select('user_id, risk_score, risk_level, reasons, drop_off_count_7d').eq('org_id', orgId).order('as_of_date', { ascending: false }).order('risk_score', { ascending: false }).limit(25),
  ])
  ...
  const avgQuizScore = quizAttempts.length > 0
    ...
    : null
  const rollupAgg = (orgRollups ?? []).reduce(
    (acc, r) => {
      acc.active += Number(r.active_learning_secs ?? 0)
      acc.idle += Number(r.idle_secs ?? 0)
      acc.total += Number(r.total_secs ?? 0)
      acc.engagement += Number(r.avg_engagement_score ?? 0)
      acc.dropoffs += Number(r.drop_off_count ?? 0)
      return acc
    },
    { active: 0, idle: 0, total: 0, engagement: 0, dropoffs: 0 }
  )
  const focusRatio = rollupAgg.total > 0 ? Math.round((rollupAgg.active / rollupAgg.total) * 100) : 0
  const avgEngagement = (orgRollups?.length ?? 0) > 0 ? Math.round((rollupAgg.engagement / (orgRollups?.length ?? 1)) * 100) : 0
  const highRiskCount = (riskSignals ?? []).filter((r) => r.risk_level === 'high').length
    ...
    <div className="p-8 max-w-6xl mx-auto space-y-10">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <a href="/api/analytics/export?scope=overview" className="text-xs px-3 py-2 rounded-lg border border-slate-700 text-slate-300 hover:text-white hover:border-slate-500 transition-colors">Export overview CSV</a>
          <a href="/api/analytics/export?scope=risk" className="text-xs px-3 py-2 rounded-lg border border-slate-700 text-slate-300 hover:text-white hover:border-slate-500 transition-colors">Export risk CSV</a>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-2">
          <p className="text-xs text-slate-400 font-medium uppercase tracking-wider">Time quality</p>
          <p className="text-3xl font-bold text-white">{focusRatio}<span className="text-lg text-slate-500">%</span></p>
          <p className="text-xs text-slate-500">Focused active time share across the last 30 rollup days.</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-2">
          <p className="text-xs text-slate-400 font-medium uppercase tracking-wider">Avg engagement score</p>
          <p className="text-3xl font-bold text-white">{avgEngagement}<span className="text-lg text-slate-500">%</span></p>
          <p className="text-xs text-slate-500">Composite score from completion, focus ratio, and assessment effort.</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-2">
          <p className="text-xs text-slate-400 font-medium uppercase tracking-wider">High-risk learners</p>
          <p className="text-3xl font-bold text-white">{highRiskCount}</p>
          <p className="text-xs text-slate-500">Learners currently flagged by the analytics risk model.</p>
        </div>
      </div>
