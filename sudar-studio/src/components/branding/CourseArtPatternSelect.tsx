    ...
    >
      <div className={cn('min-w-0', compact ? 'sm:flex-1' : undefined)}>
        <label
          htmlFor={id}
          className={cn(
            'flex items-center gap-1.5 font-medium text-slate-400',
            compact ? 'text-[11px] uppercase tracking-wide' : 'text-xs'
          )}
        >
          <Palette className="h-3.5 w-3.5 shrink-0 text-indigo-400" aria-hidden />
          {compact ? 'Default art' : 'Default course art pattern'}
        </label>
        {!compact ? (
          <p className="text-[11px] text-slate-500 leading-snug mt-1 max-w-prose">
            ...
            <span className="font-medium text-slate-300">all such courses</span> in this
      ...
      <select
        ...
        className={cn(
          'rounded-lg border border-slate-600 bg-slate-900 text-sm text-slate-100',
          'px-2.5 py-1.5 min-h-[2.25rem] focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500/50',
