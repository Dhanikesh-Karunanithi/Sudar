    ...
    <div className="space-y-4">
      ...
      {providerStatuses.length > 0 ? (
        <div className="rounded-xl border border-slate-700 bg-slate-900/50 p-4">
          <p className="text-xs font-semibold text-slate-200">Connected provider libraries</p>
          ...
          <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
            {providerStatuses.map((provider) => (
              <div key={provider.id} className="rounded-lg border border-slate-700 bg-slate-800/40 px-3 py-2">
                ...
                <p className="mt-1 text-[11px] text-slate-400">{provider.description}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          {voices.map((voice, index) => {
            ...
            return (
              <button
                ...
                className={cn(
                  'relative rounded-xl border px-4 py-3 text-left transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/70',
                  'bg-slate-900 hover:bg-slate-800/80',
                  isSelected ? 'border-indigo-400/50 shadow-[0_0_0_1px_rgba(129,140,248,0.3)]' : 'border-slate-700'
                )}
                aria-pressed={isSelected}
              >
                <p className="text-sm font-semibold text-white">{voice.name}</p>
                <p className="text-[11px] text-slate-400 mt-1">{voice.characterArchetype ?? voice.description}</p>
              </button>
            )
          })}
        </div>
          ...
          >
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="space-y-2">
                <p className="text-base font-semibold text-white">{activeVoice.name}</p>
                <div className="flex flex-wrap gap-2 text-xs">
                  {activeVoice.gender ? (
                    <span className="rounded-full border border-white/20 bg-white/10 px-2 py-1 text-slate-100">
                      {activeVoice.gender}
                    </span>
                  ) : null}
                  {activeVoice.accent ? (
                  ...
                  {(activeVoice.toneTags ?? []).map((toneTag) => (
                    <span
                      key={toneTag}
                      className="rounded-full border border-white/20 bg-white/10 px-2 py-1 text-slate-100"
                    >
                      {toneTag}
                    </span>
                  ))}
              ...
              <button
                ...
                className="inline-flex items-center gap-2 rounded-lg border border-white/30 bg-white/10 px-3 py-1.5 text-xs font-medium text-white hover:bg-white/20 focus:outline-none focus-visible:ring-2 focus-visible:ring-white/60"
            ...
            <details className="mt-4 group">
              <summary className="cursor-pointer text-xs text-slate-100/85 hover:text-white">
                See line
              </summary>
              <p className="mt-2 text-xs text-slate-100/80">“{activeVoice.sampleText}”</p>
            </details>
          </div>
        ) : null}
      </div>
